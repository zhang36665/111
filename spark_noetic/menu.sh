...
spark_tf_arm(){
	...
	echo -e "${Info}                  " 
	echo -e "${Info}请站在SPARK的正前方，与SPARK保持两米左右的距离"
	echo -e "${Info}                  " 
	echo -e "${Info}退出请输入：Ctrl + c " 
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark_tf_arm_move spark_tf_arm_move.launch camera_type_tel:=${CAMERATYPE}"
	# >>> conda initialize >>>
	# !! Contents within this block are managed by 'conda init' !!
	__conda_setup="$('/home/spark/miniconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
	if [ $? -eq 0 ]; then
    		eval "$__conda_setup"
	else
    		if [ -f "/home/spark/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/spark/miniconda3/etc/profile.d/conda.sh"
    		else
	...
	# <<< conda initialize <<<
	conda activate python3.7.1
	roslaunch spark_tf_arm_move spark_tf_arm_move.launch camera_type_tel:=${CAMERATYPE}
}

#机器人进行gazebo本体模拟仿真
spark_gazebo(){
	echo -e "${Info}                  " 
	echo -e "${Info}机器人进行gazebo本体模拟仿真" 
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark1_description spark_gazebo.launch "
...
Background_segmentation(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "开始运行背景切换，在图像中按esc退出 "
	cd ~/spark_noetic/src/spark_app/Background_segmentation_demo/script
	python3 bg_sub.py
...
navigation_grasp(){
	echo -e "${Info}" 
	echo -e "${Info}在3m*3m的地图中进行自动导航抓取" 
	ROSVER=`/usr/bin/rosversion -d`
	PROJECTPATH=$(cd `dirname $0`; pwd)
	source ${PROJECTPATH}/devel/setup.bash
	
	echo -e "${Info}                  " 
	echo -e "${Info}请先执行机械臂与摄像头标定（数字8）后再执行此程序       " 
	echo -e "${Info}退出请输入：Ctrl + c " 
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "开始运行自动导航抓取 "
	roslaunch move2grasp move2grasp.launch camera_type_tel:=${CAMERATYPE}  lidar_type_tel:=${LIDARTYPE}
